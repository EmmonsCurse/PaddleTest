...
"""
export paddle model from onnx format
"""
import time
import logging
import os
...
def load_inference_model(path_prefix, executor, model_filename=None, params_filename=None):
    ...
    if model_filename is not None and model_filename.endswith(".pdmodel"):
        ...
        [inference_program, feed_target_names, fetch_targets] = paddle.static.load_inference_model(
            path_prefix=path_prefix, executor=executor, model_filename=model_filename, params_filename=params_filename
        )
    else:
        model_name = ".".join(model_filename.split(".")[:-1]) if model_filename is not None else "model"
...
def load_onnx_model(model_path, disable_feedback=False, enable_onnx_checker=True):
    ...
    if os.path.exists(os.path.join(inference_model_path, "model.pdmodel")) and os.path.exists(
        ...
        assert os.path.exists(model_path), "Not found `{}`, please check model path.".format(model_path)
        try:
            import x2paddle
            ...
            if version_sum != 139:
                _logger.warning('x2paddle==1.3.9 is required, please use "pip install x2paddle==1.3.9".')
        ...
        try:
            ...
            version_sum = int(v0) * 100 + int(v1) * 10 + int(v2)
            if version_sum < 160:
                _logger.error('onnx>=1.6.0 is required, please use "pip install onnx".')
        except:
        ...
        if not disable_feedback:
            ConverterCheck(task="ONNX", time_info=time_info, convert_state="Start").start()
        # support distributed convert model
        model_idx = paddle.distributed.get_rank() if paddle.distributed.get_world_size() > 1 else 0
        try:
            ...
            onnx2paddle_out_dir = os.path.join(inference_model_path, "onnx2paddle_{}".format(model_idx))
            mapper.paddle_graph.gen_model(onnx2paddle_out_dir)
            _logger.info("Successfully exported Paddle static graph model!")
            if not disable_feedback:
                ConverterCheck(task="ONNX", time_info=time_info, convert_state="Success").start()
        ...
        if paddle.distributed.get_rank() == 0:
            shutil.move(
                os.path.join(onnx2paddle_out_dir, "inference_model", "model.pdmodel"),
                os.path.join(inference_model_path, "model.pdmodel"),
            )
            shutil.move(
                os.path.join(onnx2paddle_out_dir, "inference_model", "model.pdiparams"),
                os.path.join(inference_model_path, "model.pdiparams"),
            )
            load_model_path = inference_model_path
        else:
            load_model_path = os.path.join(onnx2paddle_out_dir, "inference_model")
        ...
        shutil.rmtree(os.path.join(inference_model_path, "onnx2paddle_{}".format(model_idx)))
        return val_program, feed_target_names, fetch_targets
...
def parse_args():
    ...
    parser.add_argument("--model_path", help="模型地址", default="")
